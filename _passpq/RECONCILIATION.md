...
| D2 | LAUNCH surfaces a "≥10-hour window for ComSk1" (W4/W5) (`_passla` §3.2) | **CORRECTED (verdict) — pupil-facing edit HELD for Matt** | **Measured:** LAUNCH W4 states *"the plan is used over at least ten hours across the coming weeks"* and W5 asks *"how do you show the plan was used over ~10 hours"* — i.e. LAUNCH presents a 10-hour window as a **ComSk1** expectation. **The spec places NO 10-hour requirement on Communication** (`SPEC_FACTS §15/§16`), so this over-states the unit. The correct verdict is **CORRECTED**, but the fix is a **pupil-facing teaching-content change outside the declared additive scope** — so it is **HELD, not auto-applied** (like T2-4). Proposed minimal reframe recorded in `OPEN_ITEMS`; Matt's call. |
