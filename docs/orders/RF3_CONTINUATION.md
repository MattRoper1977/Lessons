...
Checkpoint measured 09:52 UTC. The session is OPEN. This supplements, and does not
...
| #352, ML1 historical notice | `5ec70ec5a56c4814140db191bef2f33e4c94adb9` | `34686941860` | RUNNING at this checkpoint; re-read |
...
remaining generator job `103533567291` succeeded. #350 main's later generator and
separated-publication checks remain pending at this checkpoint. Two post-main shelf
...
pin are unchanged. The full build succeeded; all-file admission with controls is
RUNNING. Inputs are actual Lessons builder `810ae8f8830dc9e30a7ceec9ada4ec24d575a04d`
...
`recovery/gc1-build.log` and `gc1-admission.log`. Remaining before a merge: current
admission result and applicable GC1 gates against built content; preserve original
...
| 3 | GC1 #493 | Current build/admission/browser preflight active; content then separate P6 catalogue |
| 4 | ML1 | CLOSED-BY-DL; historical notice #352 merged, deploy pending |
...
| Added | SCI-UP1 | Read-only source intake eligible; eight named archives needed; publication queue and admissions apply |
| New | FP2 + return week | Live-exposure audit first; Code E held; original nine cards/policy and review archive required |
