/* ==========================================================================
   ART TEACH — shared interaction layer for the Art Teesside suite
   --------------------------------------------------------------------------
   Source of truth. Linked by every applicable Art Teesside lesson and hub.
   It augments authored behaviour and never replaces curriculum content.
   ...
   - Everything is namespaced under window.ArtTeach.
   ========================================================================== */
(function () {
  'use strict';

  var root = document.documentElement;
  if (root.getAttribute('data-art-teach-boot') === 'loading' || root.getAttribute('data-art-teach-boot') === 'ready') return;
  root.setAttribute('data-art-teach-boot', 'loading');
  root.setAttribute('data-art-page', document.querySelector('.slide') ? 'lesson' : 'hub');

  var AT = (window.ArtTeach = window.ArtTeach || {});
  ...
  function boot() {
    ...
    fireSlideChange();
    root.setAttribute('data-art-teach-boot', 'ready');
  }
  ...
  function setupWeDoEnhancements() {
    ...
    document.documentElement.setAttribute('data-art-teach-upgrade', 'ready');
  }
...
(function () {
  'use strict';
  var root = document.documentElement;
  if (root.getAttribute('data-art-page') !== 'hub') return;
  if (root.getAttribute('data-art-hub-ready') === 'true') return;
  root.setAttribute('data-art-hub-ready', 'true');

  var heading = document.querySelector('h1');
  var grid = document.querySelector('.grid');
  if (heading && !heading.id) heading.id = 'art-page-title';
  if (heading) document.body.setAttribute('aria-labelledby', heading.id);

  if (grid) {
    if (!grid.id) grid.id = 'art-lesson-choices';
    var skip = document.createElement('a');
    skip.className = 'art-skip-link';
    skip.href = '#' + grid.id;
    skip.textContent = 'Skip to lesson choices';
    document.body.insertBefore(skip, document.body.firstChild);

    Array.prototype.forEach.call(grid.querySelectorAll(':scope > a'), function (card, index) {
      card.classList.add('art-card-enter');
      card.style.setProperty('--art-card-index', String(Math.min(index, 16)));
      var strong = card.querySelector('strong');
      if (!card.getAttribute('aria-label') && strong) card.setAttribute('aria-label', strong.textContent.trim());
    });
  }
})();
