...
from lcontent import PROTECTED
...
print("=" * 98)
print("LSG-1 §5 ANTI-GAMING GATES · universe: the fifteen live LAUNCH v3_40min lessons")
print("=" * 98)

files = sorted(glob.glob(os.path.join(REPO, DIR, "SCI_L_W*L*.html")))
...
KEEP = {"tier panels": r'data-tier-panel="', "task boxes": r'class="box task"',
        "scaffold boxes": r'class="box scaf"', "print routes": r'class="proute',
        "print pages": r'class="pp"', "witness": r'id="print-witness"',
        "participation rows": r'class="participation"',
        "science gate": r'class="science-gate"', "entry route": r'class="entry-route"',
        "print entry": r'class="pentry"', "closure line": r'What I said, and what it changed',
        "ruled lines": r'class="pline"'}
bad3 = []
for p in files:
    now, was = open(p, encoding="utf-8").read(), base_text(os.path.basename(p))
...
outdir = os.path.join(REPO, "_lsg1", "reading", "diffs")
...
print("[PASS] plain-text prose diff written per file  %d diffs in _lsg1/reading/diffs/"
      % len(files))
