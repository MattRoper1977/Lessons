...
for (let i = 2; i < process.argv.length; i++) {
  ...
  assert.ok(['--root', '--targets', '--report', '--channel', '--artifacts'].includes(name), 'Unknown argument ' + name);
...
async function teacherAndPrint(page, host, target, touch) {
  ...
  if (artifacts && !touch) await page.pdf({path:path.join(artifacts, path.basename(target.file, '.html') + '.pdf'), format:'A4', printBackground:true});
  await page.emulateMedia({media:'screen'});
  await page.keyboard.press('Escape');
