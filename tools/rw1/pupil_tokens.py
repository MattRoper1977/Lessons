...
WEEK = re.compile(r'\bW\s?\d{1,2}\b|\bWeeks?\s?\d{1,2}\b|\bWeek of\b|\bw/c\b|\b20\d\d-\d\d\b'
...
DATE = re.compile(r'\b\d{1,2}\s+(?:%s)\.?\s+\d{2,4}\b' % MONTHS      # 19 October 2026
                  ...
                  + r'|\b20\d\d-\d{2}-\d{2}\b'                          # 2026-10-19
                  + r'|\b\d{1,2}\s+(?:%s)\b(?!\s+\d)' % MONTHS, re.I)  # 19 October (no year: CX2 Lane D found six on BUILD W8B pupil exit tickets)
