"""All-stage Friction acceptance: accessibility, keyboard, layout, enlarged text,
motion, media, no-JavaScript and print, standalone and local HTTP.

This is local browser evidence for the paired GROW W3 lesson. It is not live
publication proof, not a served-file check and not a human assistive-technology
journey. Requires Python Playwright (Chromium), PyMuPDF and axe-core at
NODE_PATH/axe-core/axe.min.js.

    NODE_PATH=/path/to/node_modules python3 tools/grow_resources/friction_acceptance.py REPO_ROOT OUT_DIR
"""
...
RESOURCE_PAGES = ['Science_Teesside/Grow/resources/GS_W3A.html', 'Science_Teesside/Grow/resources/GS_W3B.html']
TIMERS = [1, 4, 2, 9, 10, 4, 10, 32, 4, 4]
WIDTHS = [320, 390, 768, 1280]
PRINT_SECTIONS = ['ko', 'intro', 'glance', 'arrival', 'arrival-answers', 'w3a-exit', 'w3a-exit-answers', 'wedo', 'exit',
                  'witness', 'review-evidence', 'teacher-feedback', 'feedback']
...
report = {'scope': __doc__.strip(), 'stages': [], 'keyboard': [], 'enlarged': [], 'themes': [], 'media': [], 'nojs': [],
...
def axe(page, context=None):
    ...
    result = page.evaluate('''async (ctx)=>{const r=await axe.run(ctx||document,{resultTypes:['violations']});
...
def stage_checks(browser, mode, url, width, motion):
    ...
    for i in range(10):
        page.evaluate('(i)=>showSlide(i)', i); page.wait_for_timeout(120 if motion == 'reduce' else 900)  # let the entry fade finish before axe samples colours
        ...
        violations = axe(page)
        report['stages'].append({'mode': mode, 'width': width, 'motion': motion, 'index': i, 'title': title, 'focus': focus,
                                 'overflow': ov, 'axe': violations})
    ...
    # keyboard walk through every stage from the heading
    page.evaluate('()=>showSlide(0)'); page.wait_for_timeout(100)
    for i in range(9):
        # Right arrow first steps a slide's embedded model (deck-driven progression, by design)
        # and only then changes slide; press until the slide changes, bounded.
    ...
    assert page.evaluate("()=>[...document.querySelectorAll('.slide')].indexOf(document.querySelector('.slide.active'))") == 8
    ctx.close()


def enlarged(browser, mode, url):
    # 200% browser zoom is a half-width viewport at device scale 2 (1280 -> 640); the WCAG 1.4.10 reflow
    # width is 320 CSS px, checked here at double density; 200% text-only resize is the root font size.
...
def themes(browser, mode, url):
    for label, kw in [('dark', {'color_scheme': 'dark'}), ('forced-colors', {'forced_colors': 'active'})]:
        ...
        # Under forced colours the browser replaces authored colours with system colours; axe still
        # measures the authored ones, so colour-contrast is recorded there but not asserted.
...
def media(browser, mode, base):
    ...
    report['media'].append({'mode': mode, 'lessonMediaElements': lesson_media, 'runningAnimationsUnderReducedMotion': animated})
    assert lesson_media == 0
...
def nojs(browser, mode, url):
    for width in (320, 1280):
        ...
        page.goto(url)
        r = page.evaluate('''(w)=>{const q=s=>[...document.querySelectorAll(s)];const vis=e=>{const r=e.getBoundingClientRect();return r.width>0&&r.height>0&&getComputedStyle(e).visibility!=='hidden'};
          return {slides:q('.slide').filter(vis).length, arrivalQuestions:q('#arrival-supported h3,#arrival-standard h3,#arrival-stretch h3').filter(vis).length,
            exitLegends:q('#grow-w3a-exit legend').filter(vis).length, organiser:vis(document.getElementById('print-ko')), note:vis(document.querySelector('.nojs-note')),
            controls:q('.controls,.progress-label').filter(vis).length, docScrollWidth:document.documentElement.scrollWidth,
            wide:q('body *').filter(e=>vis(e)&&!(function(x){for(let a=x.parentElement;a;a=a.parentElement){const o=getComputedStyle(a).overflowX;if(o==='auto'||o==='scroll')return true}return false})(e)&&e.getBoundingClientRect().right>w+1).map(e=>e.tagName+'#'+e.id+'.'+String(e.className).slice(0,30)).slice(0,5)}}''', width)
        report['nojs'].append({'mode': mode, 'width': width, **r})
        page.screenshot(path=str(OUT / f'{mode}-nojs-{width}.png'))
        assert r['slides'] == 10 and r['arrivalQuestions'] == 12 and r['exitLegends'] == 6 and r['organiser'] and r['note'] and r['controls'] == 0 and r['docScrollWidth'] <= width + 1 and not r['wide'], r
        ctx.close()
...
def prints(browser, mode, url):
    ...
    for section in PRINT_SECTIONS:
        levels = LEVELS if section in ('arrival', 'arrival-answers', 'w3a-exit', 'w3a-exit-answers', 'exit') else ['standard']
        for level in levels:
    ...
    for level in LEVELS:
        ...
        assert not any(s in visible for s in ('print-witness', 'print-feedback', 'print-teacher-feedback', 'print-arrival-answers', 'print-w3a-exit-answers')), visible
        if mode == 'standalone':
