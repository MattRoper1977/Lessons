...
def card(row):
 ...
 return f'''<article class="card t-{row['pathway']}" data-lesson-path="{E(row['path'],quote=True)}" data-term="{E(row['term'],quote=True)}" data-style="{E(row['style'],quote=True)}" data-pathway="{E(row['pathway'],quote=True)}" data-week="{E(week_values)}"><p class="kind">{E(row['pathway'])} · Science</p><h4><a href="{E(path,quote=True)}">{E(title)}</a></h4><p class="science-week">{E(week_label)}</p><p>{E(DATA['styles'][row['style']])}</p><a class="go" href="{E(path,quote=True)}">Open lesson <span aria-hidden="true">→</span></a></article>'''
body=[]
