...
Forbidden public tokens (the gate): \bW(eek)?\s?\d+\b · \bAut(umn)?\s?[12]\b · \bSpr(ing)?\s?[12]\b · w/c · 2026-27 · "of 14" · "of 8".
...
FORBID = re.compile(r"\bW(?:eek)?\s?\d+\b|\bAut(?:umn)?\s?[12]\b|\bSpr(?:ing)?\s?[12]\b|\bw/c\b|2026[-–]27|\bof 14\b|\bof 8\b|\b(?:BUILD|GROW|LAUNCH) Weekly\s*[-–]\s*\w+'?!?\s?[A-Z]+\d+(?::[A-Z]+\d+)?", re.I)  # the last alternative: a SoW cell reference on a pupil-facing surface (staff layers only)
