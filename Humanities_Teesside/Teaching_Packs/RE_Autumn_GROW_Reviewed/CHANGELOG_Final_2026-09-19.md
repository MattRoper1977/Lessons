...
Class A (mechanical) fixes applied by _passhumd5/apply_proof.py from replacements.json: **32 edits in 27 files** (house spellings, broken joins after "means", missing spaces after punctuation, doubled full stops, DOCX heading punctuation restored). Meaning-bearing items are ledgered in _passhumd5/PROOF_LEDGER.md, not changed.
PDFs re-rendered from the proofed sources: 18 (Pupil_Resources / Knowledge_Organiser, Chromium A4).
NOT re-rendered: Teacher_Notes.pdf for GROW_RE_A2_W02, GROW_RE_A2_W03 — the container's LibreOffice cannot load any DOCX ("source file could not be loaded" on an untouched file too); their DOCX sources are proofed; re-render on a machine with a working converter.
