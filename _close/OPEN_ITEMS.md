...
| 9 | **LAUNCH ComSk1 "~10-hour window" over-claim** (W4/W5) | **CORRECTED (verdict), pupil-facing edit HELD** — the spec places **no** 10-hour requirement on Communication (`SPEC_FACTS §15/§16`); LAUNCH states one. Reframe is pupil-facing teaching content outside the declared additive scope → **not auto-applied**, awaiting Matt (like the old T2-4). Proposed minimal reframe: drop/soften the "used over ~10 hours" ComSk1 lines to pedagogy ("plan across weeks") without asserting a 10-hour assessment requirement. | `_passpq/RECONCILIATION.md` D2 |
