...
(function(){
 ...
 const title=r=>String(H.displayTitle(r)).replace(/\s*·\s*Companion pack\s*$/i,'');
 function chooser(){
 ...
 function show(pack,notes){
  ...
  main.innerHTML=`<nav class="pk-breadcrumb" aria-label="Breadcrumb"><a href="/resources/">Resources</a><span aria-hidden="true">/</span><a href="/Lessons/subject.html?subject=${encodeURIComponent(H.cardOf(pack))}">${E(subject)}</a><span aria-hidden="true">/</span><span>Lesson pack</span></nav>
   <div class="pk-intro"><div class="pk-chips">${pathway?`<span class="pk-chip ${E(pathway)}">${E(pathway)}</span>`:''}${position?`<span class="pk-chip">${E(position)}</span>`:''}</div><h1>${E(title(lesson))}</h1>${H.referenceDetail(lesson)}<p class="pk-meta">${E(['Lesson pack',halfTerm].filter(Boolean).join(' · '))}</p><div class="pk-chips">${badges.map(t=>`<span class="pk-chip pk-format">${E(t)}</span>`).join('')}</div></div>
