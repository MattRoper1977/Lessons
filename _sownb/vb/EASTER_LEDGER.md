...
## ORDER AAV-NIGHT §0 — state from the thing, and the container that was not the code

Main at read: `fa082255b170fc392e9a1d05b72d39e6d83072b8` (merge of #316, 2026-09-04T23:41:20Z). Worktree clean, no stash, designated branch `claude/aav-night-easter-campaign-a1713n` level with main.

This session's first §0 read STOPPED the order: `codex/vb-bronze-activities` (22 min old) and `codex/vb-term-navigation` (18 min old) were inside the two-hour window, and both modified `_sownb/vb/EASTER_LEDGER.md` and `_sownb/VB_STATE.json` — the two files this order requires a single writer to own. Reported as VB_BLOCKED with evidence rather than written over. Matt then confirmed the Codex run stopped; the order was re-issued and resumed at §0. No ledger line was written while blocked, deliberately: appending to a file two live branches held was the collision the rule exists to prevent.

**§1's premise is stale, and this is the correction.** The order describes a stacked, unmerged Codex chain to recover and re-target. It is already landed. #304 and #305–#313 merged as SQUASHES (content on main as `e2fb2770`, `0ef914e3`, `df0c49f2`, `174338c0`, `897dcbcd` and siblings); #314/#315/#316 as merge commits. The Silver PR #308's base was `main`, NOT `codex/aae-explore-batch1` — nothing was stacked, so nothing needs re-targeting. The `codex/aae-*` branch tips read "not contained" only because a squash rewrites the commit; that is a history artefact, not lost work. No open PR exists from that Codex session at all.

**MEASUREMENT INVALID on arrival, and it was the container.** The mechanism battery reds on main: 29 tools, 118 controls, eleven tools reporting `declared=0 fired=n/a`. The cause was not code. `VB_STATE.capability.sessionLocalInstalls` records these installs as container-local and says to re-run them in a fresh container; this is a fresh container. Restoring `lxml`, `pymupdf`, `pillow`, `cffi` and `poppler-utils` moved 118 → 130; the record, however, UNDER-RECORDS ITS OWN DEPENDENCIES — `cssselect` and `pypdf` are required by eleven tools and named in neither list. With those two added the battery reports **29 tools, 358 controls, every declared control fired, PASS** — exactly the 358 the previous session recorded, which is the evidence that the harness never changed and only the environment had. `--prove-red` re-armed it: planting a `lesson_stages` expectation of 9999 makes the broken tool exit 1 and the battery red. Recorded so the next fresh container does not spend the same hour: the true install set is `lxml cssselect pypdf pymupdf pillow cffi` plus `poppler-utils`.

**§1c LEAK CHECK — clean.** 231 files landed across the AAE chain (`12920b29..e81ffd6e`). No `Games/`, `Site/` or `Apps/` path appears in any of them; the Apex leak the order anticipated did not happen. Three paths fall outside the declared Art_Teesside / tools/artsaward / docs / catalogue scope and all three are legitimate, printed here as the reverse the order asks for: `.github/workflows/fieldops-p2-and-sweep.yml` (the estate's own VB gate job, which is where the new g30 step belongs — and its `shell: bash -Eeo pipefail` means that step cannot fail silently, checked rather than assumed); `tools/verify_cross_estate_unification.py` (a single `resources.json` manifest re-pin, mechanically required by the catalogue edit); and `index.html`, which REMOVED an over-claim — the hub badge "ARTS AWARD MAPPED ✓ · AQA UAS ALIGNED" and the assertion that BUILD=Explore/GROW=Bronze/LAUNCH=Silver became "ARTS AWARD · TEESSIDE ART" with pathway-neutral buttons. Nothing reverted; nothing quarantined; the leak list is empty.

## Mechanism repair, alone — g28 read SVG geometry as a workbook cell

g28's bare-cell pattern `C(\d{2,3})` matched the `C` of an SVG cubic-curve command. Demonstrated on main, not argued: `BUILD <svg><path d="M0 0 C210 10 30 40 50 60"/><text>C999</text></svg>` returned TWO citations — `'BUILD Weekly - ?'!C210` alongside the real `C999`. A must-fire control returning a false red is a control failing on main, which is the one condition §8c opens the mechanism for, so this lands alone and carries no lesson unit.

The repair is the Codex session's own, adopted rather than rewritten so its work is credited and not dropped (§1e). It masks only `<path d="…">` values whose content is entirely path-charset, and SPACE-FILLS rather than deletes so every later citation offset stays correct. It ships with two controls of its own: SVG *visible text* must still cite, and an `aria-label` citation must still cite — so the mask cannot quietly swallow the accessible label it was never meant to touch. Both fire; `--self-test` PASS at 4/4.

**Honest scope, measured not assumed:** across all 42 shipped award decks the fix changes NOTHING — main's g28 and the repaired g28 both see 0 citations, because those decks carry none. The misfire is real but LATENT; it bit a Codex Bronze draft, and §4's floor of ≥3 rendered visuals per deck guarantees it would bite this campaign. Landing a proven, controlled repair before the work that triggers it, rather than after.

§9 checked, not asserted: `tools/verify_fixture_names.mjs` ALLOW entries counted at **28**, unchanged and untouched; the tree reports no person-shaped fixture strings. No threshold moved. One file changed.

## The second defect the first one exposed: g28's controls fired nowhere

Adding g28 to the battery was refused by the battery itself, and the refusal was the finding. **g28 was in neither the mechanism battery nor CI.** The workflow names it only inside a comment on the g29 step. Its `--self-test` existed and passed, and nothing ever ran it — a gate whose controls fire nowhere is a gate in name only, which is the exact condition §12-S3 calls silent on main.

The cause is one level up from any single gate. `mechanism_battery.py` opens by explaining that nothing in it is pinned, that "a tool that adds a control needs no change here" — and that is true of CONTROL COUNTS and false of the TOOL SET. `SELFTEST_TOOLS` and `LIST_ONLY_TOOLS` are hand-typed literals. Control counts are derived; the roster is not. So a gate is invisible to the battery until somebody remembers to type its name, and g28 was never typed. The battery was written to catch gates that cannot fire and could not see one standing beside it.

Repaired minimally, and no further: `g28_cell_existence.py` added to `SELFTEST_TOOLS`, and g28 taught to publish itself in the shape the battery reads — `--list-controls`, plus the `N/M controls fired` summary line the battery parses to learn how many a tool actually ran. Counting follows g30's convention, where "fired" means BEHAVED AS DECLARED: g28 carries a negative control (`citesARealAddress`, mustFire=False) which earns its place by staying silent, and counting it only when it trips would score a working control as a failure.
...
## Codex continuation — 5 September 2026

PR #320 merged at `cb8dd8c836c305ba112f5167919a1f8a41416bf1` after 8/8 applicable checks passed on `ab3503d4213ef118209f39575488b5de190d088f`. The full merge tree equals the reviewed tree. The original candidate failed the W7 boundary solely because these two campaign notes accompanied the watch workflow. The repair was narrowed to that workflow; its notes are restored above in the campaign transaction. No W7 fence or served game bytes changed. Pages, FieldOps, W7 and Watch subsequently passed.

### Correction to the interim census

The 596/221/375 readback used the spine's stale absolute-week field. Twenty-three Spring 2 week-six cells are explicitly NOT-TIMETABLED under the existing ruled-week function. The correct boundary is **573 scoped, 221 lexically recognised, 352 open for semantic review**. The original 262-plan file and all coverage thresholds remain unchanged. These open candidates must be checked against actual existing teaching; they are not an instruction to manufacture 352 missing lessons. Exact excluded references and the source warning are in `evidence/bronze_activities/scope_correction.json`.

### Bronze activity revision batch — fourteen units

Recovered the source draft and checked the five known review findings against it. Repaired two further stale cues, then replaced seven primary tasks with genuine labelling, prediction and error-finding work. The final sequence contains six teaching shapes; maximum count is three, no consecutive shape repeats, and every six-deck window contains at least four shapes. Interactive implementation uses native select, radio and ordering controls with the same pupil questions/options in print and a separate teacher key.

All fourteen rebuilds pass comparative content checks, including g18/g23/g25/g26/g28/g29, with zero donor sentence leaks. g16/g19/g24 remain explicitly PRE-EXISTING; this batch does not claim those historical chassis-contract conflicts resolved. All 136 actual-spec DOM/schema controls pass. Browser interactions and visual PDF review are release gates and are still pending at this checkpoint. The fourteen existing canonical lesson URLs, award parts, source plans, 40-minute allocations and nine-stage structure remain bound. This is fourteen revised units, zero new workbook-cell coverage.


## Bronze released; original Science measurement repaired

#321 merged at `6555277013b3719997659e9642968434fc981731`. All seven applicable checks were green on `b364aaffa97fd299823986e3a964e0f3ead47715`; all 48 changed paths are identical in the merge tree. Browser: 255/255, including the previously failing tablet toolbar-clearance case. Pupil print: 42 pages visually inspected, zero clipping or blank pages. Pages, FieldOps and Watch passed after merge. Fourteen revised units; no new workbook-cell coverage.

The next Science review demonstrated a measurement defect on unchanged main: all 25 original Science routes use `body > div.slide-container`, while lesson_stages recognises only main-based roots. The gates therefore see zero stages and zero words; g23 prints a zero-load pass. The repair recognises that exact original container form, excludes nested print lookalikes and already-main-owned containers, and reads the actual original `data-timer` clock where canonical `data-min` is absent. Seven new controls cover the original shape, inactive carousel stages, double counting, hidden/print containers and timer attributes. No learner page, original plan, band, ceiling, fixture allowlist or contract row is changed in this repair. Measured before/after source identities and the complete battery are in `evidence/original_shell_measurement/`. The subsequent Science batch must be assessed with this repaired reader; zero-word passes are not its evidence.

The same blind spot existed independently in g26. Its additional root fallback now recognises only the visible body-child container, while preserving existing main-root precedence and its complete text-extraction rules. Seven additional controls cover the root, staff exclusion, inactive carousel stages, hidden roots and print lookalikes. All 481 previously recognised main-layout files produce identical reading measurements; all 25 original Science sources now return actual pupil scores. The battery passes 31 tools / 382 controls and proves a planted failure is detected. The original BUILD sources exceed the BUILD reading band before any proposed lesson repair; these are now measured inherited findings, not missing-text errors.

Post-merge Bronze HTTP verification also completed: every one of the fourteen live pages matched its reviewed SHA256. Source, URL and returned-body identities are retained in `evidence/bronze_activities/served321.json`.

## Original Science release candidate — original routes, two real periods

PR #322 merged as `2631d986a68d734d2a7fd6534c8841546273bf1d`, six applicable checks green on exact head `d0a402b5507accd57efdbd0924f2272aa90b2227`; complete merge tree equals the reviewed tree.

The recovered twenty-five original Science repairs were replayed against their exact source SHA256 and then reviewed against the actual pupil tasks. Ten BUILD/GROW routes retain two separate forty-minute periods with a working period-two restart. Fifteen LAUNCH routes total forty minutes each; three receive substantive practical handover repairs. The manual independent clock is the sole active clock and pauses on leaving its stage. This is thirteen substantive revised routes and twelve timing-only repairs, zero new cells.

The review shortened the new BUILD task instructions in the screen and all print tiers, kept technical vocabulary and model limits, and corrected over-broad older food-chain and plate claims. The arm model is an access route in the earlier tasks too. All twenty-five now pass the unchanged reading bands; the original five BUILD sources measured 4.52–5.93 before these edits, and the revised five measure 3.29–3.93. Maximum g23 ratio is 0.99, below the stricter 1.25 order target. Every gate ran on both source and revised bytes; no pass-to-fail regression remains. Existing floor, token, visual/declaration and one optional external-link finding are explicit in `evidence/science_originals/comparative.json`. All 119 inline scripts parse.

Browser/print status at this candidate: PENDING. The new job uses installed Chrome, the reviewed Site HUD, all twenty-five actual routes, native buttons and print CSS; it exports twenty-four representative PDFs and their page images for visual review. No browser pass, classroom duration or practical completion is asserted before that review. Sources, exact adjustments and identities are retained under `evidence/science_originals/`.

## Original Science released; Explore activity batch prepared

#323 merged as `436a6028c62ed5bf1fd8b579c34dbe892b884d04`: all seven applicable checks passed on `c4ead4488a20606d43c5e9276690eacc0f0e6d62`, and all 43 changed paths match the reviewed merge tree. Browser: 213/213 cases over 25 routes and 33 viewport cases. Print: 24 exports / 123 pages visually reviewed. The last review found the older exclusive food/job claim in nutrition and traced it through the model answer, active narration, food cards and exit; all are now corrected together. The final run has 114 pixel-identical previously reviewed pages and nine changed nutrition pages, all inspected. See `evidence/science_originals/final_release_review.json`. Live HTTP confirmation follows deployment. Thirteen substantive revisions and twelve timing-only repairs, zero new cells.

Explore: fourteen existing routes now have genuine native primary activities across six shapes. Counts are 3/3/2/2/2/2; no adjacent repeat, and every six contains at least five shapes. All fourteen comparative builds pass with inherited g16/g19/g24 explicit, zero donor sentence leaks, and 136/136 actual-spec DOM/schema checks. BUILD pupil reading is 2.97–3.98 with unchanged bands and chrome treatment. The connection lesson now teaches with two expressly made-up concrete notes and plain pupil objectives; real artist and arts group exploration and exact record locations remain required. Other practical work, print choices, source plans, nine stages and forty-minute totals are preserved; the only additional independent-stage edit fixes an article typo. This is fourteen revisions and zero new cells. Browser and print release gates are pending, with per-deck review and exact changes under `evidence/explore_activities/`.

## Explore released; Silver activity batch prepared

#324 merged as `0e47feef67e18858278c9620f49a3222e39c3e38`, eight applicable checks green on `7e9dadce9d96845dfc1551d3e91d10d8fd0c6b66`; all 47 changed paths match the reviewed tree. Explore browser 255/255 and Bronze regression 255/255. All 42 Explore print pages inspected. An initial CI red identified a subjectless aggregate PASS in the new Science release report; the report now names its 25 exact source files and eight print representatives, without altering the sweep grammar. Fourteen revised units, zero new cells.

Science post-merge Pages, FieldOps, unification and Watch checks all passed. The GitHub hostname now returns a 301 to `madebymatt.uk`. The twenty-five direct body confirmations remain open: the original request read redirect pages, while following the custom domain encountered a runtime network-approval cancellation. This is not recorded as a lesson-content defect or an unredirected serving pass. Evidence is in `evidence/science_originals/postmerge_status.json`.

Silver: fourteen primary activities now use six genuine shapes, with counts 3/3/2/2/2/2, no adjacent repeats and at least five distinct shapes per six. The measured source had twelve sort/match and two commit/reveal declarations. Every other stage, objective, allocation, timer, practical task, independent task and print choice is unchanged. Actual attendance, first-hand encounter, personal reviews and real leadership delivery remain required. Fourteen comparative builds and 136 DOM/schema checks pass; pupil reading is 5.38–7.00 within the unchanged GROW band. Two error-finding prompts now use the classifier’s existing wording while retaining their actual decisions. This is fourteen revisions and zero new cells. Browser and print verification remain release gates.

## Silver native activities and the stricter award-load finding — draft release, 2026-09-05

The extra raw-ratio audit caught seven Bronze, two Explore and eleven Silver routes above the operative 1.25 target (one Silver ratio rounded to 1.25 but was 1152/918.5). The general g23 CLI ceiling of 1.5 was never evidence that these met 1.25. Fourteen Silver activity revisions plus nine existing Bronze/Explore prose repairs spend 23 units in this batch, with zero new coverage cells.

Concise explanation replaces repetition in named stages; worked art methods, real practice, source attribution, nine-stage forty-minute timing, independent pupil judgement and qualification evidence remain. The Bronze review-frame example now requires a source before asserting material, application or audience. Silver 3005 needed clearer sentence boundaries after trimming exposed FK 7.12; the content was repaired rather than changing the band. All 23 comparative builds pass, as do all 42 actual reading bands and raw load ratios, with identical family medians. DOM/schema checks pass 136/136 per strand. Browser interaction and page-image review are still release gates. Evidence: _sownb/vb/evidence/silver_activities/.

Silver/award-load release review: eight checks pass at df91e289. Browser acceptance is 255/255 in each strand (765/765 total), with no page errors or unexpected external requests. All 42 Silver print pages were inspected; the other 84 pages rasterise identically to the already inspected Bronze and Explore releases. Artifact 9968911021, SHA256 5dd8b21db137f84c84f6f46a767384a333c027166da6908dd6ad3e98deb1f3cf. The final evidence-only head still requires fresh CI before merge.

## #325 merged; download recovery becomes an exact-file release gate — 2026-09-05

PR #325 merged at 7287a3fe from 09e88b6a after all eight final checks passed. The complete merge tree equals the reviewed tree, 5bcbfa8a, across 68 changed paths. Pages, FieldOps and Watch main then passed. All forty-two award routes now have genuine varied native tasks and pass their unchanged reading bands and the stricter raw 1.25 load target.

The recovered packer had never been driven offline. Its menu labels came from filenames; one required local source-card page was absent; local pack-start links and four cross-term lesson links would break after extraction. The integrated draft now reads actual lesson titles, explicitly includes the source-card page and two linked staff resources, creates local pack homes and four honest online-continuation pages, and refuses all other unresolved local navigation. It does not rewrite source lessons. The exact reviewed Site HUD is bundled only in offline copies with the narrow loader/navigation adaptation.

Local preflight: twelve reproducible ZIPs, 130 declared lessons, three support pages, 45 packaging/HUD controls, current source-based term placement and complete Science main/alternative census. New workflow Lesson downloads is registered with the watch in this same change; its trigger-list control passes. Exact extracted-file browser acceptance and visual pack-start review are required before ZIP publication in the separate final navigation/catalogue transaction. These are packaging tools and copies of existing lessons, zero authoring units and zero new coverage cells.

## Download browser finding and CI ownership, 5 September 2026
