...
# VB-RUN14 R2.2: a capitalised pair resolving to SUBJECT_TERMS never holds a lesson.
# Derived from the workbook cells (no spec vocabulary exists in the repo); NAMED-AFTER
# and subject-name rows are in the same bulleted lists and resolve the same way.
SUBJECT=places_registry(ROOT/'_sownb/SUBJECT_TERMS.md')
...
def classify(value,public,examples):
 ...
 if tokens & SUBJECT:return 'SUBJECT-TERM'
 return 'UNRESOLVED'
...
def main():
 ...
 synthetic=''.join(('Test','given'))+' '+''.join(('Test','family'));red=candidates(text+' '+synthetic,public,examples);red_fired=any(r['candidate']==synthetic and r['classification']=='UNRESOLVED' for r in red);role=' '.join(('Learner','Name'));role_green=classify(role,public,examples)=='ROLE-LABEL';place=' '.join(('Newport','Bridge'));place_green=classify(place,public,examples)=='PLACE';house=' '.join(('Exit','Ticket'));house_green=classify(house,public,examples)=='HOUSE-LABEL';demonym=' '.join(('Diverse','British'));demonym_green=classify(demonym,public,examples)=='PLACE';subject=' '.join(('Solar','System'));subject_green=classify(subject,public,examples)=='SUBJECT-TERM';structural=["Week Timeline","Now Record","Role Evaluation"];structural_green=all(classify(value,public,examples)=='ROLE-LABEL' for value in structural);hygiene=[register_hygiene('PLACES',PLACES-PLACES_BUILTIN-DEMONYMS),register_hygiene('HOUSE_LABELS',HOUSE),register_hygiene('DEMONYMS',DEMONYMS),register_hygiene('SUBJECT_TERMS',SUBJECT)];hygiene_clean=all(h['clean'] for h in hygiene);hygiene_control=not register_hygiene('planted',DEMONYMS|{'the'})['clean'];nonvacuous=bool(text) and red_fired and role_green and structural_green and place_green and house_green and demonym_green and subject_green and hygiene_control
 report={'gate':'g10-role-classification','file':a.file,'sha256':hashlib.sha256(path.read_bytes()).hexdigest(),'searchSpace':'pupil-facing main.deck after scripts, styles, SVG and keyed staff guidance are removed','corpusChars':len(text),'candidates':rows,'unresolved':unresolved,'controls':{'unresolvedRedFired':red_fired,'roleGreen':role_green,'placeGreen':place_green,'placeRegisterTokens':len(PLACES),'houseGreen':house_green,'houseRegisterTokens':len(HOUSE),'demonymGreen':demonym_green,'demonymRegisterTokens':len(DEMONYMS),'subjectGreen':subject_green,'subjectRegisterTokens':len(SUBJECT),'registerHygiene':hygiene,'registerHygieneClean':hygiene_clean,'registerHygieneControlFired':hygiene_control,'structuralRoleLabels':structural,'structuralRoleGreen':structural_green,'nonVacuous':nonvacuous},'s23':'MEASUREMENT INVALID by design; S23_NAMES is prohibited','status':'PASS' if nonvacuous and hygiene_clean and not unresolved else 'RED'};out=ROOT/a.output;out.parent.mkdir(parents=True,exist_ok=True);out.write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({'status':report['status'],'candidates':rows,'unresolved':unresolved,'controls':report['controls']},indent=2));return 0 if report['status']=='PASS' else 1
