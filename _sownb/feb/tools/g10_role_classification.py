...
PLACES={"earth","teesside","stockton","tees","estuary"}
def registry(path):
...
def main():
 ...
 synthetic=''.join(('Test','given'))+' '+''.join(('Test','family'));red=candidates(text+' '+synthetic,public,examples);red_fired=any(r['candidate']==synthetic and r['classification']=='UNRESOLVED' for r in red);role=' '.join(('Learner','Name'));role_green=classify(role,public,examples)=='ROLE-LABEL';structural=["Week Timeline","Now Record","Role Evaluation"];structural_green=all(classify(value,public,examples)=='ROLE-LABEL' for value in structural);nonvacuous=bool(text) and red_fired and role_green and structural_green
 report={'gate':'g10-role-classification','file':a.file,'sha256':hashlib.sha256(path.read_bytes()).hexdigest(),'searchSpace':'pupil-facing main.deck after scripts, styles, SVG and keyed staff guidance are removed','corpusChars':len(text),'candidates':rows,'unresolved':unresolved,'controls':{'unresolvedRedFired':red_fired,'roleGreen':role_green,'structuralRoleLabels':structural,'structuralRoleGreen':structural_green,'nonVacuous':nonvacuous},'s23':'MEASUREMENT INVALID by design; S23_NAMES is prohibited','status':'PASS' if nonvacuous and not unresolved else 'RED'};out=ROOT/a.output;out.parent.mkdir(parents=True,exist_ok=True);out.write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({'status':report['status'],'candidates':rows,'unresolved':unresolved,'controls':report['controls']},indent=2));return 0 if report['status']=='PASS' else 1
