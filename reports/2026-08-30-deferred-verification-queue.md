...
## V6PGF STRUCK register — six conclusions, not six quiet corrections

These are the six conclusions the V6-PG run got wrong before a firing or inverse
control overturned them. They are **STRUCK**, not superseded. The historical text
below is retained only so the error and the control that caught it remain
auditable.

STRUCK-1
claim: Apex Velodrome `OVERWRITTEN` another game's passport.
control: a deployed-writer seed, re-read before navigation, carried `lamport: 7`; the synthetic under-populated seed could not distinguish rejection from clobber.

STRUCK-2
claim: Marble, Apex Velodrome and Grapple wiped a valid child's passport on boot.
control: marking only through `mutateProfile` and `grantAward` made all seven valid-record arms preserve every value; the planted drop-key build still red, proving the value predicate was live.

STRUCK-3
claim: an ungestured boot write is itself destructive, so deployed Apex Kick violates C1.
control: deleting one seeded key red C1 while unmodified Apex Kick performed its boot write and preserved every pre-boot value; C1 therefore judges values, not write count.

STRUCK-4
claim: replacement-only C2a was a sufficient test of the corrupt-backup obligation.
control: removing Apex Curl's backup line left the replacement-only reading green because Curl defaults in memory; the read-aware exact-byte control red, while unmodified Curl greened.

STRUCK-5
claim: the old Apex literal regex's green on Curl and red on Velodrome were product verdicts.
control: an in-memory fixture proved the old regex missed a planted `mbm_*` write and counted both a migration read and an export filename; the call-site classifier caught and separated all three roles (2/2 expected outcomes).

STRUCK-6
claim: Neon Turf never reads the passport because `grep -c -F 'PASSPORT_KEY'` found constants only.
control: the storage read spy observed the indirect `STORAGE.passport` read at `renderMenuStats`; the corrupt raw string remained in storage, separating read from discard.

### V6PGF citation census

found: 17 citations in scope — 10 current tracked-text occurrences, all in this
queue's visibly STRUCK or HISTORICAL evidence, plus 7 immutable commit messages.
cleared: 17/17. No current contract, verifier, report conclusion or deployment
decision cites a struck claim as true.

immutable-history map:

- `ef44e23ac1a66d7c4193e9ab950eb8d4f7f4ed05` → STRUCK-1
- `d80d7d92f03e2d2780de7d2037736c31000734a3` and `0d324c2c9b0b23fdd58e0ab91b3d7b3a891cf64e` → STRUCK-2
- `0ae6405b16414f71186ba0cd62ce6f368de66b1c` and `485c1f54987d66ce57323131a26c9e069fa563b5` → STRUCK-3
- `006fba62eec498f09f24527569b6f7f75d42690f`, `485c1f54987d66ce57323131a26c9e069fa563b5` and `1b42bf046977e6eb60c30a34a0ab1fc0bc20aa0c` → STRUCK-4
- `d80d7d92f03e2d2780de7d2037736c31000734a3` → STRUCK-5
- `485c1f54987d66ce57323131a26c9e069fa563b5` and `1b42bf046977e6eb60c30a34a0ab1fc0bc20aa0c` → STRUCK-6

Some immutable messages name more than one struck conclusion; the census counts
distinct commit messages, not map entries.

---

## 5a-STRUCK-1. A withdrawn reading, struck rather than left to be re-cited
...
**Citation-census correction:** the earlier paragraph claimed no committed
artefact carried `OVERWRITTEN`; commit `ef44e23a` does carry it while striking
it. The consolidated census above records that immutable historical citation and
the control that clears it. Nothing downstream depends on the old verdict.

---

## 5a-STRUCK-2. The passport-reset finding is WITHDRAWN — I made the same mistake twice
...
**This is the same failure the STRUCK-1 marker above already caught**, and the
...
What survives from §5a, re-derived under the corrected V6PGF instrument, is the
value contract: boot writes are judged by preservation of the deployed-writer
record, and every corrupt default path must preserve the exact raw bytes under
the deterministic backup key. The `mbm-default00000000` node id was real only
on fresh installs; V6PGF repaired both affected candidates and proved C5 by
generation, persistence and format rather than by inspecting a seeded record.

---

## 5a-HISTORICAL — STRUCK-2. The shared sports passport reset conclusion

**Status: STRUCK-2. Historical text retained as evidence of the bad instrument;
do not cite any conclusion in this subsection as a current product finding.**
...
**Historical hold resolved:** the valid runtime-marked arms preserve every
seeded value, the planted drop-key build still reds, and the repaired candidates
are governed by the V6PGF contract rather than this struck conclusion.

---

## 5b-STRUCK-5. The old Apex storage-literal verdicts are not product verdicts
...
**Repaired under V6-PG/V6PGF.** The old literal regex remains historical evidence
only. The corrected call-site classifier separates writes, migration reads and
export filenames. Its two inverse fixtures both produced the expected result:
a planted hidden `mbm_*` write was caught, while a migration read plus export
filename was classified without becoming a write (2/2 controls).
...
## 5c. Neon Turf V6 drops hud.js (CONFIRMED; repaired by V6PGF)
...
V6PGF restored the exact canonical HUD script line and the focused three-viewport
render probe proved the repaired control visible, on top and at least 44×44.

---

## 5d. V6-PG §6/§7 — the graft each held build needs (C2)

Recorded per §7.3 as historical provenance for the grafts V6PGF subsequently
applied. **STRUCK-3:** the write-count C1 proxy and its STOP are withdrawn. The
binding C1 test is value preservation: an unmodified deployed Apex Kick boot
write greens, while deleting one seeded value reds and identifies the loss.
...
**STRUCK-6 — a correction to this table's own first version.** It said Neon Turf "never
...
**STRUCK-4:** the C2a/C2b split was useful diagnosis but replacement-only C2a is
not a sufficient contract. Removing Apex Curl's backup left that reading green
because Curl defaults only in memory. The final contract is singular: whenever a
build proceeds with a default after reading corrupt passport data, it preserves
the exact raw bytes at `mbm_sports_passport_v4_corrupt_backup` before proceeding.
The read-aware inverse fixture catches a removed backup line; unmodified and
repaired candidates preserve the bytes exactly.
