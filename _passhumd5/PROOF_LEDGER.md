...
| H5 | B0126, B0130 | **RE-MEASURED after the edits (E6 POST, same instrument, 120 × 9 × 2)** | B0126 closed: `#previous-slide` / `#next-slide` / organiser button no longer below 4.5:1 (they were the LAUNCH #3b82f6 buttons); svg-img-alt 35 → 0, nested-interactive 26 → 0, select-name 4 → 0; color-contrast 1080 → 120 stage-visits per viewport, all on the model-node ribbon (B0125, report only). B0130 therefore stays literally true — no lesson is free of a serious finding — for that one ruled-report-only row. |
