...
byte for byte, against the committed original. If that passes, the layer cannot
have altered a single character of lesson content.

```
$ python3 _framework/qa_check.py
Checked 31 decks against origin/main

PASS · lesson content byte-identical in all decks
PASS · current framework present in all decks
```

Run it before every commit. It exits non-zero on failure so it can gate one.
...
- **Zone token contrast.** None of the four zone colours clears 4.5:1 against
  ...
  revisited.
- **Legacy CSS from the source template.** Each deck carries rules for
  components it does not use — `.bio-block`, `.hl-sentence`, `.fact-carousel`,
  `.bridge-banner` (whose label still names two Art lessons), `.misc-box`. Dead
  weight, not visible to a class. Removing it means editing lesson files, which
  is a separate job from adding a layer.
- **`.pres-cap` text colour** is a hardcoded indigo from that same template
  rather than a subject token, so it is the one element that does not follow its
  deck's palette. Left as-is: changing it is a palette decision.
